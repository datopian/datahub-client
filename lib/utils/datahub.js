...
class DataHub extends EventEmitter {
  ...
  async pushFlow(specPath){
    ...
    // Attach dp.json
    const dp_url = spec.inputs[0].url
    const dp_res =  await fetch(dp_url)
    if (dp_res.status === 200) {
      descriptor = await dp_res.json()
    } else {
      throw new Error(responseError(dp_res))
    }
      ...
      if (sheets === 'all') {
        ...
        sheets.forEach(sheet => {
          if (workbook.SheetNames.length < sheet) {
            throw new Error(`sheet index ${sheet} is out of range`)
          }
        })
      } else { // Default case
        sheets = [1]
